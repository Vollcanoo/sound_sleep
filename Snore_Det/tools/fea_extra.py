...
if __name__ == '__main__':
    ...
    dataset_root = repo_root / "Snoring_Dataset"
    class_dirs = ["1", "0"]
    audioExt = ['wav', 'WAV']
    input_sample = None
    output_sample = 16000
    audioDirectory = [dataset_root / "orgin" / class_name for class_name in class_dirs]
    audio_outputDirectory = [dataset_root / "resample_16k" / class_name for class_name in class_dirs]
    fea_outputDirectory = [dataset_root / "fea" / class_name for class_name in class_dirs]

    for directory in audio_outputDirectory + fea_outputDirectory:
    ...
    for i, dire in enumerate(audioDirectory):
        ...
        for file in tqdm.tqdm(clean_speech_paths, desc='No.{} dataset resampling'.format(i)):
            ...
            audio_outputFileName = os.path.join(str(audio_outputDirectory[i]), fileName)
            ...
            fea_outputFileName = os.path.join(str(fea_outputDirectory[i]), fea_fileName)
