...
static const char *SYSTEM_PROMPT =
    ...
    "决策优先级：\n"
    "1. 若之前已充气，且鼾声消失（snore_detected=false），优先输出 deflate both，intensity=70，duration_sec=9。\n"
    "2. 翻身中（MOVING）时输出 hold。\n"
    "3. 头不在枕上（NO_HEAD）时输出 hold。\n"
    "4. 无鼾声（snore_detected=false）且之前未充气时输出 hold。\n"
    "5. 仰卧(SUPINE) + 鼾声严重(snore_minutes_per_hour>=4)：inflate right，intensity 60-80。\n"
    "6. 仰卧 + 鼾声中等(2-4分钟/时)：inflate right，intensity 40-50。\n"
    "7. 仰卧 + 鼾声轻微(<2分钟/时)：hold。\n"
    "8. 左侧卧 + 鼾声严重：inflate left，intensity 50-70。\n"
    "9. 右侧卧 + 鼾声严重：inflate right，intensity 50-70。\n"
    "10. max_probability>0.9 且 positive_window_ratio>0.1：非常严重，intensity 70-80。\n"
    "11. confidence<0.5 时保守处理，降低 intensity。\n\n"
